...
void Anal_Leptop_PROOF::SlaveBegin(TTree * /*tree*/)
{
  ...
  // The tree argument is deprecated (on PROOF 0 is passed).
  TString option = GetOption();
  //OutFile = new TProofOutputFile("testdata_sel18A_ejet17v.root");
  //OutFile = new TProofOutputFile("QCD_HT2000toInf_mumu2021Feb.root");
  OutFile = new TProofOutputFile("SMu_2018D_mumu2021Feb.root");
...
Bool_t Anal_Leptop_PROOF::Process(Long64_t entry)
{
...
#ifdef Data_MC
  //if((isTT && DiLeptt && MUMU)) return kFALSE;
#endif
  ...
  */
  ...
  float muptarr[njetmx];
  

  for(int imu=0; imu<nmuon1; imu++) {
    
    if(muonpt[imu]<35.) continue;
  ...
  if (nelec1 != 0) return kFALSE;
  hist_count->Fill(5,weight);
 
  std::vector<TLorentzVector> mumupair;
  double pt_mu1(-99.), pt_mu2(-99);
  int itf(-1), iuf(-1);
  float fmu1_ch(-99.), fmu2_ch(-99.);
  if (nmuon2>=2) {
    double chkmu1pt(0.), chkmu2pt(0.);
    //for(unsigned int it=0; it<mucandv.size(); it++){
    for(int lp=0; lp<nmuon2; lp++){
      chkmu1pt = muptarr[lp];
      //chkmu1pt = mucandv[it].Pt();
      if (chkmu1pt > pt_mu1) {
	pt_mu1 = chkmu1pt;
	itf = lp;
      }
    }
    //for(unsigned int iu=0; iu<mucandv.size(); iu++){
    for(int lp2=0; lp2<nmuon2; lp2++){
      if (lp2!=itf) {
      //if (iu!=itf) {
	chkmu2pt = muptarr[lp2];
	//chkmu2pt = mucandv[iu].Pt();
	if (chkmu2pt > pt_mu2) {
	  pt_mu2 = chkmu2pt;
	  iuf = lp2;
	  //iuf = iu;
  ...
  if ((fmu1_ch*fmu2_ch)>0) return kFALSE;
  hist_count->Fill(8,weight);
  
  if ((mucandv[itf] + mucandv[iuf]).M() < 20.) return kFALSE;
  if ((mucandv[itf] + mucandv[iuf]).M() > 76. && (mucandv[itf] + mucandv[iuf]).M() < 106.) return kFALSE;
  ...
  if (PFMET<70.) return kFALSE;
  hist_count->Fill(12,weight);
  
  //Fill Histograms with base level dilepton selection set//
  ...
  hist_init[7]->Fill((mucandv[itf] + mucandv[iuf]).M(),weight);
  hist_init[8]->Fill(mucandv[itf].Pt(),weight);
  hist_init[9]->Fill(mucandv[itf].Eta(),weight);
  hist_init[10]->Fill(mucandv[itf].Phi(),weight);
  hist_init[11]->Fill(mucandv[iuf].Pt(),weight);
  hist_init[12]->Fill(mucandv[iuf].Eta(),weight);
  hist_init[13]->Fill(mucandv[iuf].Phi(),weight);
  ...
  hist_initpuwup[7]->Fill((mucandv[itf] + mucandv[iuf]).M(),weight_puwup);
  ...
  hist_initpuwup[11]->Fill(mucandv[iuf].Pt(),weight_puwup);
  hist_initpuwup[12]->Fill(mucandv[iuf].Eta(),weight_puwup);
  hist_initpuwup[13]->Fill(mucandv[iuf].Phi(),weight_puwup);
  ...
  hist_initbtagwup[7]->Fill((mucandv[itf] + mucandv[iuf]).M(),weight_btagwup);
  ...
  hist_initbtagwup[11]->Fill(mucandv[iuf].Pt(),weight_btagwup);
  hist_initbtagwup[12]->Fill(mucandv[iuf].Eta(),weight_btagwup);
  hist_initbtagwup[13]->Fill(mucandv[iuf].Phi(),weight_btagwup);
  ...
  hist_initpuwdown[7]->Fill((mucandv[itf] + mucandv[iuf]).M(),weight_puwdown);
  ...
  hist_initpuwdown[11]->Fill(mucandv[iuf].Pt(),weight_puwdown);
  hist_initpuwdown[12]->Fill(mucandv[iuf].Eta(),weight_puwdown);
  hist_initpuwdown[13]->Fill(mucandv[iuf].Phi(),weight_puwdown);
  ...
  hist_initbtagwdown[7]->Fill((mucandv[itf] + mucandv[iuf]).M(),weight_btagwdown);
  ...
  hist_initbtagwdown[11]->Fill(mucandv[iuf].Pt(),weight_btagwdown);
  hist_initbtagwdown[12]->Fill(mucandv[iuf].Eta(),weight_btagwdown);
  hist_initbtagwdown[13]->Fill(mucandv[iuf].Phi(),weight_btagwdown);
  ...
  if (npfjetAK8_mu>=1)  hist_count->Fill(14,weight);
  if (npfjetAK8_el>=1)  hist_count->Fill(15,weight);
  if (npfjetAK8_mu>=2)  hist_count->Fill(16,weight);

  int thcand = -1;
  int nhcand = 0;
  int tmucand1(-1);
  bool tmu_found1(false);
  int nmucand1(0);
  float maxmass_mu(-200.);
  
  for(int ijet=0; ijet<npfjetAK8; ijet++){
    if (pfjetAK8_hasmatchmu[ijet] == 1) {
      if (pfjetAK8mass[ijet] > maxmass_mu) {
	maxmass_mu = pfjetAK8mass[ijet];
	tmucand1 = ijet;
	tmu_found1 = true;
	nmucand1++;
      }
  ...
  if (tmucand1>=0) {
    ...
    //hist_obs_muljesup[1]->Fill(pfjetAK8y[tmucand1],weight);                                                                
    //hist_obs_muljesdown[1]->Fill(pfjetAK8y[tmucand1],weight);                                                                
    ...
    /*
      if(thcand>=0){
      hist_obs_mu[14]->Fill(pfjetAK8sdmass[thcand],weight);
      }
      hist_obs_mu[15]->Fill(pfjetAK8haspfelectron[tmucand1],weight);
      hist_obs_mu[16]->Fill(pfjetAK8haspfmuon[tmucand1],weight);
      if(pfjetAK8muon[tmucand1]>=0){
      hist_obs_mu[17]->Fill(delta2R(pfjetAK8y[tmucand1],pfjetAK8phi[tmucand1],muoneta[pfjetAK8muon[tmucand1]],muonphi[pfjetAK8muon[tmucand1]]),weight);
      }
      int idaugh_in = 0;
      for(int ip=0; ip<idp; ip++){
      if(delta2R(pfjetAK8y[tmucand1],pfjetAK8phi[tmucand1],genparteta[top_dp[ip]],genpartphi[top_dp[ip]])<0.8){
        hist_obs_mu[18]->Fill(abs(genpartpdg[top_dp[ip]]),weight);
        Idaugh_in++;
	}
	}
	hist_obs_mu[19]->Fill(idaugh_in,weight);
	int leptop_found = 0;
	for(int ileptop=0; ileptop<nleptop; ileptop++){
	if(delta2R(pfjetAK8y[tmucand1],pfjetAK8phi[tmucand1],leptop4v[ileptop].Rapidity(),leptop4v[ileptop].Phi()) < 0.8){    
        leptop_found = 1;
        break;
	}
	}
	hist_obs_mu[20]->Fill(leptop_found,weight);
	hist_obs_mu[22]->Fill(pfjetAK8rt[tmucand1],weight);*/
  }
  
  int tmucand2(-1);                                                                                 
  bool tmu_found2(false);                                                                           
  int nmucand2(0);
  float maxmass2(-200.);
  ...
  for(int ijet=0; ijet<npfjetAK8; ijet++){                                                          
    if (pfjetAK8_hasmatchmu[ijet] == 1) {
      if (ijet != tmucand1) {
	if (pfjetAK8mass[ijet] > maxmass2) {                                                        
	  maxmass2 = pfjetAK8mass[ijet];                                                            
	  tmucand2 = ijet;                                                                          
	  tmu_found2 = true;                                                                        
	  nmucand2++;                                                                               
	}
  ...
  if (tmucand2>=0) {
    ...
    hist_obs[0]->Fill(pfjetAK8pt[tmucand2],weight);       
    hist_obs[1]->Fill(pfjetAK8y[tmucand2],weight);                                                  
    hist_obs[2]->Fill(pfjetAK8mass[tmucand2],weight);                                               
   
    hist_obs[3]->Fill(pfjetAK8NHadF[tmucand2],weight);                                              
    hist_obs[4]->Fill(pfjetAK8neunhadfrac[tmucand2],weight);                                        
    hist_obs[5]->Fill(pfjetAK8sdmass[tmucand2],weight);                                             
    hist_obs[6]->Fill(pfjetAK8chrad[tmucand2],weight);                                              
    hist_obs[7]->Fill(pfjetAK8subhaddiff[tmucand2],weight);                                         
    hist_obs[8]->Fill(pfjetAK8tau21[tmucand2],weight);                                              
    hist_obs[9]->Fill(pfjetAK8DeepTag_TvsQCD[tmucand2],weight);                                     
    hist_obs[10]->Fill(pfjetAK8_bbyW_E[tmucand2],weight);                                           
    hist_obs[11]->Fill(pfjetAK8_Kfactor[tmucand2],weight);                                          
    hist_obs[12]->Fill(pfjetAK8re_tvsb[tmucand2],weight);                                           
    hist_obs[13]->Fill(pfjetAK8rnu_tvsb[tmucand2],weight);                                           
    
    hist_obspuwup[0]->Fill(pfjetAK8pt[tmucand2],weight_puwup);
    hist_obspuwup[1]->Fill(pfjetAK8y[tmucand2],weight_puwup);
    hist_obspuwup[2]->Fill(pfjetAK8mass[tmucand2],weight_puwup);
    hist_obspuwup[3]->Fill(pfjetAK8NHadF[tmucand2],weight_puwup);
    hist_obspuwup[4]->Fill(pfjetAK8neunhadfrac[tmucand2],weight_puwup);
    hist_obspuwup[5]->Fill(pfjetAK8sdmass[tmucand2],weight_puwup);
    hist_obspuwup[6]->Fill(pfjetAK8chrad[tmucand2],weight_puwup);
    hist_obspuwup[7]->Fill(pfjetAK8subhaddiff[tmucand2],weight_puwup);
    hist_obspuwup[8]->Fill(pfjetAK8tau21[tmucand2],weight_puwup);
    hist_obspuwup[9]->Fill(pfjetAK8DeepTag_TvsQCD[tmucand2],weight_puwup);
    hist_obspuwup[10]->Fill(pfjetAK8_bbyW_E[tmucand2],weight_puwup);
    hist_obspuwup[11]->Fill(pfjetAK8_Kfactor[tmucand2],weight_puwup);
    hist_obspuwup[12]->Fill(pfjetAK8re_tvsb[tmucand2],weight_puwup);
    hist_obspuwup[13]->Fill(pfjetAK8rmu_tvsb[tmucand2],weight_puwup);

    hist_obsbtagwup[0]->Fill(pfjetAK8pt[tmucand2],weight_btagwup);
    hist_obsbtagwup[1]->Fill(pfjetAK8y[tmucand2],weight_btagwup);
    hist_obsbtagwup[2]->Fill(pfjetAK8mass[tmucand2],weight_btagwup);
    hist_obsbtagwup[3]->Fill(pfjetAK8NHadF[tmucand2],weight_btagwup);
    hist_obsbtagwup[4]->Fill(pfjetAK8neunhadfrac[tmucand2],weight_btagwup);
    hist_obsbtagwup[5]->Fill(pfjetAK8sdmass[tmucand2],weight_btagwup);
    hist_obsbtagwup[6]->Fill(pfjetAK8chrad[tmucand2],weight_btagwup);
    hist_obsbtagwup[7]->Fill(pfjetAK8subhaddiff[tmucand2],weight_btagwup);
    hist_obsbtagwup[8]->Fill(pfjetAK8tau21[tmucand2],weight_btagwup);
    hist_obsbtagwup[9]->Fill(pfjetAK8DeepTag_TvsQCD[tmucand2],weight_btagwup);
    hist_obsbtagwup[10]->Fill(pfjetAK8_bbyW_E[tmucand2],weight_btagwup);
    hist_obsbtagwup[11]->Fill(pfjetAK8_Kfactor[tmucand2],weight_btagwup);
    hist_obsbtagwup[12]->Fill(pfjetAK8re_tvsb[tmucand2],weight_btagwup);
    hist_obsbtagwup[13]->Fill(pfjetAK8rmu_tvsb[tmucand2],weight_btagwup);

    hist_obspuwdown[0]->Fill(pfjetAK8pt[tmucand2],weight_puwdown);
    hist_obspuwdown[1]->Fill(pfjetAK8y[tmucand2],weight_puwdown);
    hist_obspuwdown[2]->Fill(pfjetAK8mass[tmucand2],weight_puwdown);
    hist_obspuwdown[3]->Fill(pfjetAK8NHadF[tmucand2],weight_puwdown);
    hist_obspuwdown[4]->Fill(pfjetAK8neunhadfrac[tmucand2],weight_puwdown);
    hist_obspuwdown[5]->Fill(pfjetAK8sdmass[tmucand2],weight_puwdown);
    hist_obspuwdown[6]->Fill(pfjetAK8chrad[tmucand2],weight_puwdown);
    hist_obspuwdown[7]->Fill(pfjetAK8subhaddiff[tmucand2],weight_puwdown);
    hist_obspuwdown[8]->Fill(pfjetAK8tau21[tmucand2],weight_puwdown);
    hist_obspuwdown[9]->Fill(pfjetAK8DeepTag_TvsQCD[tmucand2],weight_puwdown);
    hist_obspuwdown[10]->Fill(pfjetAK8_bbyW_E[tmucand2],weight_puwdown);
    hist_obspuwdown[11]->Fill(pfjetAK8_Kfactor[tmucand2],weight_puwdown);
    hist_obspuwdown[12]->Fill(pfjetAK8re_tvsb[tmucand2],weight_puwdown);
    hist_obspuwdown[13]->Fill(pfjetAK8rmu_tvsb[tmucand2],weight_puwdown);

    hist_obsbtagwdown[0]->Fill(pfjetAK8pt[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[1]->Fill(pfjetAK8y[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[2]->Fill(pfjetAK8mass[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[3]->Fill(pfjetAK8NHadF[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[4]->Fill(pfjetAK8neunhadfrac[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[5]->Fill(pfjetAK8sdmass[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[6]->Fill(pfjetAK8chrad[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[7]->Fill(pfjetAK8subhaddiff[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[8]->Fill(pfjetAK8tau21[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[9]->Fill(pfjetAK8DeepTag_TvsQCD[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[10]->Fill(pfjetAK8_bbyW_E[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[11]->Fill(pfjetAK8_Kfactor[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[12]->Fill(pfjetAK8re_tvsb[tmucand2],weight_btagwdown);
    hist_obsbtagwdown[13]->Fill(pfjetAK8rmu_tvsb[tmucand2],weight_btagwdown);

    hist_obsljesup[0]->Fill(pfjetAK8pt[tmucand2]*pfjetAK8jesup_total[tmucand2],weight);
    hist_obsljesdown[0]->Fill(pfjetAK8pt[tmucand2]*pfjetAK8jesdn_total[tmucand2],weight);
    //hist_obsljesup[1]->Fill(pfjetAK8y[tmucand2],weight);                                                                   
    //hist_obsljesdown[1]->Fill(pfjetAK8y[tmucand2],weight);                                                                  
 
    hist_obsljesup[2]->Fill(pfjetAK8mass[tmucand2]*pfjetAK8jesup_total[tmucand2],weight);
    hist_obsljesdown[2]->Fill(pfjetAK8mass[tmucand2]*pfjetAK8jesdn_total[tmucand2],weight);
    
    hist_obsljerup[0]->Fill(pfjetAK8pt_resoup[tmucand2],weight);
    hist_obsljerdown[0]->Fill(pfjetAK8pt_resodown[tmucand2],weight);
    hist_obsljerup[2]->Fill(pfjetAK8mass_resoup[tmucand2],weight);
    hist_obsljerdown[2]->Fill(pfjetAK8mass_resodown[tmucand2],weight);
    ...
    hist_obs[15]->Fill(pfjetAK8haspfelectron[tmucand2],weight);                                     
    hist_obs[16]->Fill(pfjetAK8haspfmuon[tmucand2],weight);                                         
    if(pfjetAK8muon[tmucand2]>=0){                                                                  
      hist_obs[17]->Fill(delta2R(pfjetAK8y[tmucand2],pfjetAK8phi[tmucand2],muoneta[pfjetAK8muon[tmucand2]],muonphi[pfjetAK8muon[tmucand2]]),weight); 
    }                                                                                               
    int idaugh_in = 0;                                                                              
    for(int ip=0; ip<idp; ip++){                                                                    
      if(delta2R(pfjetAK8y[tmucand2],pfjetAK8phi[tmucand2],genparteta[top_dp[ip]],genpartphi[top_dp[ip]])<0.8){
    ...
    int leptop_found = 0;                                                                           
    for(int ileptop=0; ileptop<nleptop; ileptop++){                                                 
      if(delta2R(pfjetAK8y[tmucand2],pfjetAK8phi[tmucand2],leptop4v[ileptop].Rapidity(),leptop4v[ileptop].Phi()) < 0.8){                             
	leptop_found = 1;                                                                           
	break;                                                                                      
      }                                                                                             
    }                                                                                               
    hist_obs[20]->Fill(leptop_found,weight);                                                        
    hist_obs[22]->Fill(pfjetAK8rt[tmucand2],weight);
    /*
  ...
  if (npfjetAK8_mu>=1 && npfjetAK8_el>=1) hist_count->Fill(17,weight);
#endif
