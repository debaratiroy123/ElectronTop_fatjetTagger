...
void Anal_Leptop_PROOF::SlaveBegin(TTree * /*tree*/)
{
  ...
  OutFile = new TProofOutputFile("SMU2018D_emu2021Marchallwt100mettopov1.root");
...
Bool_t Anal_Leptop_PROOF::Process(Long64_t entry)
{
  ...
  if (npfjetAK8>0) {
	...
	if( (delta2R(mucandv[itf].Eta(),mucandv[itf].Phi(),pfjetAK8y[ijet],pfjetAK8phi[ijet]) > 1.2)
      ...
      if (telcand>=0) hist_init[6]->Fill(npfjetAK8wmass,weight);
      if (telcand>=0) hist_count->Fill(13,weight);
      if (telcand>=0 && pfjetAK8_hasmatche[telcand] == true) hist_count->Fill(14,weight);
	...
	if (ijet != telcand && pfjetAK8_hasmatche[ijet] == false) {
      ...
      if (tmucand1>=0) hist_count->Fill(15,weight);
      if (tmucand1>=0 && pfjetAK8_hasmatchmu[tmucand1] == true) {
	...
	hist_obs_muljerdown[2]->Fill(pfjetAK8mass_resodown[tmucand1],weight);
      ...
      if (telcand>=0 && pfjetAK8_hasmatche[telcand] == true) {
